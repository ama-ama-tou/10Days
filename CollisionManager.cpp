...
void  CollisionManager::blockCollision(Player& player_, Block**& block_) {
	for (int r = 0; r < maxRow_; r++) {
		for (int c = 0; c < maxCol_; c++) {
			...
			player_.getImageLtPos();
			Vec2 a = block_[r][c].getLtVertex();

			a.x -= block_[r][c].getLocalCo().getOrigin().x;
			a.y -= block_[r][c].getLocalCo().getOrigin().y;

		/*block_[r][c].setPrePos(a);*/
	...
	for (int r = 0; r < maxRow_; r++) {
		for (int c = 0; c < maxCol_; c++) {
			if (block_[r][c].getIsHadBlock() == true) {
				...
				Novice::ScreenPrintf(10, 620, "rbro_:%d", rbro_);
				Novice::ScreenPrintf(10, 640, "cbro_:%d", cbro_);
			}
		}
	}
